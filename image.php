...
if (isset($_POST["register"])) {
    ...
    $carwash = preg_replace("/[^a-zA-Z0-9_]/", "", $_POST['Store']); // sanitize table name
    ...
    // Insert profileowner
    $stmt = $conn->prepare("INSERT INTO profileowner (image, name, username, passwordOw, district, city, fulladdress, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $uniqueName, $fullName, $carwash, $password, $district, $address, $fulladdress, $description);

    if (!$stmt->execute()) {
        die("Error inserting profile: " . $stmt->error);
    }

    // Create dynamic booking table
    $createTableSQL = "
    ...
    if (!$conn->query($createTableSQL)) {
        die("Error creating booking table: " . $conn->error);
    }
